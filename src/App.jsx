import { useState } from 'react'
import reactLogo from './assets/react.svg'
import viteLogo from '/vite.svg'
import './App.css'

function App() {
  const [count, setCount] = useState([
    {
      name: 'Find your Best Match',
      title: 'cupiditate, quod iusto omnis fuga provident maxime. Architecto, minima aut'
    },
    {
      name: 'Find your Best Match',
      title: 'cupiditate, quod iusto omnis fuga provident maxime. Architecto, minima aut'
    },
    {
      name: 'Find your Best Match',
      title: 'cupiditate, quod iusto omnis fuga provident maxime. Architecto, minima aut'
    },
  ])

  return (
    <>

      <div className='flex justify-end'>

        <ul className="flex  flex-wrap  flex-row items-center p-3">
          <li className='mx-3 '>Browse Profile</li>
          <li className='mx-3 '>Member Login</li>
          <li className='mx-3 bg-blue border-slate-950 w-28 rounded hover:bg-amber-800 bg-amber-600'><button className='p-1 text-white'>Register Free</button></li>
        </ul>
      </div>
      <div className='w-screen border-slate-950 relative overflow-hidden' >
        <div className='overflow-hidden'>

          <img className='w-screen  h-screen' src="data:image/jpeg;base64," alt="" />
        </div>


      </div>


      <div class="reserve mb-8">

        <div class="tables">
          <div class="reserve1"> <select name="time" id="table" class="res">
            <option value="">👨‍👧1 People</option>
            <option value="">👨‍👧2 People</option>
            <option value="">👨‍👧3 People</option>
            <option value="">👨‍👧4 People</option>
            <option value="">👨‍👧5 People</option>
            <option value="">👨‍👧6 People</option>
            <option value="">👨‍👧7 People</option>
            <option value="">👨‍👧8 People</option>
          </select></div>
          <div class="reserve1 " > <select name="time" id="time" class="res">
            <option value="">1 AM</option>
            <option value="">2 AM</option>
            <option value="">3 AM</option>
            <option value="">4 AM</option>
            <option value="">5 AM</option>
            <option value="">6 AM</option>
            <option value="">7 AM</option>
            <option value="">8 AM</option>
          </select></div>
          <div class="reserve1" > <input type="date" name="" id="" class="res" /></div>
          <div class="reserve1" >Select Table</div>
        </div>
      </div>




      <div className='w-5/12 mx-auto mt-10 text-center font-bold text-xl'>Lorem ipsum dolor sit amet, cmet aliquid ullam debitis commodi, quam tempora molestiae neces
        !</div>


      <div className='w-1/2  mx-auto text-center my-3'>
        Lorem ipsum dolor sit amet consectetur adipis
        icing elit. Hic voluptate cupiditate, quod iusto omnis fuga provident maxime. Architecto, minima aut.

      </div>



      <div className='  flex justify-evenly h-96 mt-10 ' >

        {count.map((items) => {
          return (
            <>
              <div className='w-72 h-48  before:opacity-0.1 mm'>
                <button className='w-10 m-2  bg-amber-600 h-11 outline-none '></button>

                <h4 className='text-xl font-bold p-2'>{items.name}</h4>
                <p className='text-black p-2'>{items.title}</p>
              </div>

            </>
          )

        })}
      </div>
    </>
  )
}

export default App
